...
function isOffscreen(node) {
  if (node.resolved) {
    return (
      !node.detached && (node.y > WINDOW_HEIGHT || node.y + node.height < 0)
    );
...
function prettyPrintNode(node, offscreenChildren, offscreenViews) {
  ...
  console.groupCollapsed(
    `FOUND: %c${offscreenViews} %cattached offscreen views (from ${offscreenChildren.length} / ${node
  ...
  while (index < ancestors.length) {
    ...
    const showIndex =
      parent.index !== -1 && parent.parent.children.length !== 1;
  ...
  buffer.text += print(
    index + 3,
    `${offscreenChildren.length} / ${node.children
      .length} children here are offscreen.`,
  );
  buffer.text += print(
    index + 3,
    `This currently amounts to ${offscreenViews} total native views`,
  );
  buffer.text += print(
    index + 3,
    'You should consider setting `removeClippedSubviews` to true on the ',
  );
  buffer.text += print(
    index + 3,
    'on the containing View in order to improve scroll performance.',
  );
  ...
  for (let i = 0; i < children.length; i += 1) {
    ...
    buffer.text += print(
      index,
      `%c<${child.name}%c${props} %c/>%c${warning}%c${post}`,
    );
    buffer.styles.push(
      COMPONENT_COLOR,
      PROPS_COLOR,
  ...
  while (index > 0) {
    ...
    buffer.styles.push(COMPONENT_COLOR);
  }
